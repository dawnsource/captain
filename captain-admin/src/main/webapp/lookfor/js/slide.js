...
function getImg(id){
  ...
  if(!document.getElementById(id)){
	...
	i.onmouseover = function (){
		...
		$("#itemTextTip").empty().html($("#"+id+'tt').html());
		$("#itemTextTip").fadeIn('normal');
	}
	i.onmouseout = function (evt){
		...
		if(obj != null && obj.nodeName.toLowerCase() == 'div' && obj.className != null && obj.className != '' &&
			(obj.className == 'imgtipnva' || obj.className == 'itemTitle' || obj.className == 'itemInfo' || obj.className == 'itemInfo001' ||
			 obj.className == 'itemInfo002' || obj.className == 'itemContent' || obj.className == 'itemMore')) {
			//ÔÝ²»²Ù×÷
		} else {
			clearInterval(i.tiptimer);
			autoSlide();

			$("#itemTextTip").empty().fadeOut('normal');
		}
	};
	i.onclick = function (){unfoldOrshrink(0);};
	i.className = imgclass;
    i.style.opacity = 0;
    i.style.filter = 'alpha(opacity=0)';
    i.src = imgdir + '/' + document.getElementById(id+'t').lang;
  }else{
