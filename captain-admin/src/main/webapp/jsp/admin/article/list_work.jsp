...
<form id="appfrm" class="form-search" action="${ctx}/admin/products!list.action" method="post">
<!--  隐藏域 -->
  <s:token />
  ...
  <div id="search_div" class="well">
      <div class="row">
		<div class="control-group span4">
		  <label><s:text name="admin.label.comm.chinese.title"/>：</label>
		  <input type="text" class="input-large" placeholder="" id="query.title" name="query.title" value="${query.title}">
		</div>
		<div class="control-group span4">
		  <label><s:text name="admin.label.comm.english.title"/>：</label>
          <input type="text" class="input-large" placeholder="" id="query.etitle" name="query.etitle" value="${query.etitle}">
		</div>
	</div>
    <div class="row">
...
<%--        <a class="btn" href="javascript:void(0);" onclick="doExport();"><s:text name="global.button.label.export"/></a>--%>
        <a class="btn" href="${ctx}/admin/products!edit.action?domain.type=${query.type}&treeid=${param.treeid}" onclick=""><s:text name="global.button.label.add"/></a>
...
<%--        <th nowrap="nowrap"><s:property value="%{#attr.isAuditPage ? getText('admin.thead.comm.author') : getText('admin.thead.comm.status')}"/></th>--%>
        <th nowrap="nowrap"><s:property value="getText('admin.thead.comm.author')"/></th>
    ...
    <s:iterator var="item" value="page.datas" status="stat">
	 <tr ${stat.even ? '' : 'class=""'}>
        ...
        <td align="center">
            <a href="${ctx }/admin/products!${isAuditPage?'detail':'edit' }.action?oid=${item.oid}" onclick="doEdit('${item.oid}','${isAuditPage?'detail':'edit' }');return false;">
              ${stat.count }
            </a>
        </td>
        <td align="center">
	    ...
	    <td align="center">
          <s:property value="%{#attr.isAuditPage ? #item.authorname : 'status'"/>
	    </td>
        <td align="center">
...
function doDelete(delstyle){
	...
	if(confirm('<s:text name="global.hintinfo.deleteconfirm"/>')){
		...
		if(delstyle == 1){
			$('#appfrm')[0].action = '${ctx}/admin/products!move2trash.action';
		} else if(delstyle == 2){
			$('#appfrm')[0].action = '${ctx}/admin/products!delete.action';
		}
		$('#appfrm').submit();
	}
}
...
function doEdit(oid,method){
	...
	$('#appfrm')[0].action = '${ctx}/admin/products!'+method+'.action';
	$('#appfrm').submit();
}
function doExport(){
