...
<s:set value="%{getText('admin.navi.headline.title.management.about.collaboration')}" var="pagetitle"/>
...
<form id="appfrm" class="form-search" action="${ctx}/admin/aboutcollaboration!list.action" method="post">
<!--  隐藏域 -->
  <s:token />
...
<%--        <a class="btn" href="javascript:void(0);" onclick="doExport();"><s:text name="global.button.label.export"/></a>--%>
        <a class="btn" href="${ctx}/admin/aboutcollaboration!edit.action?domain.type=${query.type}&treeid=${param.treeid}" onclick=""><s:text name="global.button.label.add"/></a>
  ...
  <table class="table table-striped">
    ...
    <s:iterator var="item" value="page.datas" status="stat">
	 <tr ${stat.even ? '' : 'class=""'}>
        ...
        <td align="center">
            <a href="${ctx }/admin/aboutcollaboration!${isAuditPage?'detail':'edit' }.action?oid=${item.oid}" onclick="doEdit('${item.oid}','${isAuditPage?'detail':'edit' }');return false;">
              ${stat.count }
            </a>
        </td>
        <td align="center">
...
function doDelete(delstyle){
	...
	if(confirm('<s:text name="global.hintinfo.deleteconfirm"/>')){
		...
		if(delstyle == 1){
			$('#appfrm')[0].action = '${ctx}/admin/aboutcollaboration!move2trash.action';
		} else if(delstyle == 2){
			$('#appfrm')[0].action = '${ctx}/admin/aboutcollaboration!delete.action';
		}
		$('#appfrm').submit();
	}
}
...
function doEdit(oid,method){
	...
	$('#appfrm')[0].action = '${ctx}/admin/aboutcollaboration!'+method+'.action';
	$('#appfrm').submit();
}
function doExport(){
	location = '${ctx}/admin/aboutcollaboration!exportData.action?'+$('#appfrm').serialize();
}
jQuery.fn.extend({
